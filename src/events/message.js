...
module.exports = async function messageEvent(app, event, dispatcher) {
  ...
  app.db.touchUser(user, event.senderName, { countMessage: app.config.features.messageCounter !== false });

  const thread = app.db.getThread();
  if (app.config.features.messageCounter !== false) thread.totalMessages += 1;
  ...
  if (math) {
    if (math.expiresAt <= Date.now()) {
      ...
      if (Number(body) === math.answer) {
        ...
        app.db.scheduleSave();
        await dispatcher.safeSend('🧠 صحيح! حصلتِ على 50 عملة.', event.threadID);
      } else {
        user.gameLosses += 1;
        app.db.scheduleSave();
  ...
  if (reply?.messageID) {
    ...
    if (entry && String(entry.threadID) === String(event.threadID)) {
      const allowed = entry.allowEveryone || !entry.userID || String(entry.userID) === String(event.senderID);
      if (!allowed) return true;
      if (entry.commandName === 'ذكاء') {
        await dispatcher.runCommand(
          event,
          { name: 'ذكاء', args: [], raw: body },
          { fromReply: true, replyMode: true, parent: entry }
        );
        return true;
      }
    }

...
function extractParticipants(event) {
  ...
  return raw.map((item) => ({
    ...
    name: cleanName(item.fullName || item.name || item.firstName, 'عضوة')
  })).filter((item) => item.id);
}
...
module.exports = async function threadEvent(app, event, dispatcher) {
  ...
  if (!['log:subscribe', 'participantAdded', 'subscribe'].includes(type)) return false;
  ...
  await dispatcher.safeSend(
    box('أهلًا بكِ', [
      ...
      `اكتبي ${app.config.bot.prefix}القائمة لرؤية ما أستطيع فعله.`,
      'المطور: Rin Il'
    ]),
    event.threadID
  );
